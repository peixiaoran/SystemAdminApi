...
namespace SystemAdmin.Repository.FormBusiness.CompreQuery
{
    public class GeneralQueryRepository
    {
        ...
        public GeneralQueryRepository(SqlSugarScope db, Language lang)
        {
            _db = db;
            _lang = lang;
        }
        ...
        public async Task<ResultPaged<FormQueryDto>> GetGeneralQueryPage(GetGeneralQueryPage getPage)
        {
            ...
            var page = await BuildQuery(getPage).ToPageListAsync(getPage.PageIndex, getPage.PageSize, totalCount);
        ...
        /// <param name="getPage"></param>
        /// <returns></returns>
        public async Task<DataTable> GetGeneralQueryExcel(GetGeneralQueryPage getPage)
        {
            return await BuildQuery(getPage).ToDataTableAsync();
        }
        ...
        /// 组装综合查询（分页与导出共用）
        /// </summary>
        private ISugarQueryable<FormQueryDto> BuildQuery(GetGeneralQueryPage getPage)
        {
            ...
            // 排序：按申请日期倒序
            query = query.OrderBy((instance, dic, formtype, applyuser, applydept, search) => instance.ApplicantDate, OrderByType.Desc);

            return query.Select((instance, dic, formtype, applyuser, applydept, search) => new FormQueryDto
            {
